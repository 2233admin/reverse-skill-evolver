"""Read-only capability inventory for the newest usable local IDA.

The script intentionally does not install, upgrade, remove, register, or alter IDA.
It is the canonical Python implementation used by the package CLI.
"""
...
from .ida import find_latest_ida, ida_release_version
...
def build_report(args: argparse.Namespace) -> dict[str, Any]:
    ...
    repository_reference = package_root.parent / "skills" / "ida-reverse" / "references" / "ida-plugin-capabilities.json"
    reference = repository_reference if repository_reference.is_file() else package_root / "data" / "ida-plugin-capabilities.json"
    capabilities = json.loads(reference.read_text(encoding="utf-8"))

    installation = find_latest_ida(
        [args.ida_dir] if args.ida_dir else (),
        only_candidates=bool(args.ida_dir),
    )
    ida_dir = Path(installation.install_dir) if installation else None
    ida_exe = Path(installation.executable) if installation else None
    ida_gui = ida_exe
    ida_cfg = ida_dir / "cfg" / "idagui.cfg" if ida_dir else None
    ida_found = bool(ida_exe and ida_exe.exists() and installation and Path(installation.idalib_path).is_file())
    ida_gui_found = bool(ida_gui and ida_gui.exists())
    ida_version = installation.version if installation and installation.version else "unknown"
    ...
    plugin_validation = validate_plugin_tree(
        plugin_root,
        ida_version=ida_release_version(ida_version),
        ...
        plugin_rows.append(
            {
                ...
                "compatibility": spec.get("compatibility", "smoke-test against the selected IDA installation"),
    ...
    tool_specs: list[dict[str, Any]] = [
        {"name": "ida-pro", "kind": "binary", "path": str(ida_exe) if ida_exe else None, "available": ida_found, "version": ida_version if ida_found else None, "note": "Newest discovered IDA installation; interactive sign-in is handled by the operator when required."},
        ...
        {"name": "git-ida", "kind": "cli", "path": str(teams_cli) if teams_cli and teams_cli.exists() else None, "available": bool(teams_cli and teams_cli.exists()), "version": git_ida_version, "note": "Teams Git-backend driver from the selected IDA installation; repository initialization remains explicit."},
    ]
    native_features: list[dict[str, Any]] = [
        {
            ...
            "gates": ["GUI mode for Jump Anywhere, Pathfinder, and Xrefs Graph widgets"],
        ...
        },
        {
            ...
            "gates": [],
            "use": "Prefer the selected IDA's native rustc/crate/panic/calling-convention recovery before optional readability plugins.",
        },
        {
            "name": "ida94-swift",
            ...
            "gates": ["Hex-Rays decompiler for pseudocode"],
        ...
        },
        {
            ...
            "automation": "gui_only",
            "mcp_tools": [],
            "gates": ["interactive GUI mode", "a local Dyld Shared Cache input"],
            "use": "Use the selected IDA's dedicated DSC widgets and component navigation; do not claim headless support without a matching MCP API.",
        },
        {
            "name": "ida-teams",
    ...
    return {
        ...
        "ida": {"root": str(ida_dir) if ida_dir else None, "executable": str(ida_exe) if ida_exe else None, "gui_executable": str(ida_gui) if ida_gui else None, "config": str(ida_cfg) if ida_cfg else None, "found": ida_found, "gui_found": ida_gui_found, "selected_version": ida_version},
...
def write_report(report: dict[str, Any], output_dir: Path) -> None:
    ...
    lines = [
        ...
        f"Selected IDA: **{ida.get('selected_version') or 'unknown'}**  |  headless: **{ida['found']}**  |  GUI: **{ida.get('gui_found', False)}**  |  HCLI: **{bool(discovery['hcli'])}**  |  MCP listening ports: **{', '.join(map(str, discovery['online_mcp_ports'])) or 'none'}**",
        "",
    ...
    lines += ["", "## Selected IDA native features", "", "| Feature | Available | Automation | State | Gates | Use |", "|---|---|---|---|---|---|"]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Read-only capability inventory for the newest usable local IDA")
    ...
    repository_generated = Path(__file__).resolve().parent.parent / "skills" / "generated"
    output_dir = Path(args.output_dir) if args.output_dir else repository_generated
