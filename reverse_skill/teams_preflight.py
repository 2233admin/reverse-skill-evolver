#!/usr/bin/env python3
"""Read-only readiness probe for the newest local IDA Teams Git backend.
...
from .ida import find_latest_ida
...
def find_git_ida(explicit_path: str = "") -> Path | None:
    ...
    candidates = [
        ...
        str(Path(installation.install_dir) / "tools" / "teams" / "git-ida.exe") if installation else None,
    ]
