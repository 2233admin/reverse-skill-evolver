...
> Markdown 字段契约。机器可读枚举与 ID 模式由 `reverse_skill/data/case-contracts.json`
> 冻结（本文件是人工镜像）；`reverse-skill case review` 是唯一规范校验入口。
...
（notes 含 `offline` / `离线` / `not applicable` 时才允许 `n/a` / `n/a_re`）。

**SHA-256 校验**（`reverse-skill case review --verify-hashes`）：

- `content_hash` 接受 `sha256:<64 hex>` 或裸 `64 hex`；`n/a` 表示无文件 artifact。
- `artifact_path` 必须解析到 case root 内；**逃逸 case root 是 error**（路径逃逸防护，
  fail-closed），绝对路径在 root 外同样报错。
- hash 不匹配是硬失败（`artifact.hash_mismatch`）。
...
**MUST**：`evidence_ids` 非空；`status=validated` 时 confidence 不得为 low。
...
完整敏感内容只在用户项目报告中；journal **MUST** 脱敏（`anonymization.md`），并过
`reverse-skill gates leak-scan`。
