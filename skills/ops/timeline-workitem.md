...
> 可回放作战记录 + 覆盖勾选（WorkItem 思想）。
> 全部落在 **`work/<case>/`**（仓库 gitignore），不进 skill 包正文。
> 机器可读枚举由 `reverse_skill/data/case-contracts.json` 冻结（本文件是人工镜像）。
...
work/<case>/
  ...
  workitems.md       # 工作项与覆盖
  evidence/          # 原始产物（截图、pcap、日志、E-*.md 记录）
  notes/
  report/            # 最终报告草稿或拷贝
```
...
```bash
reverse-skill case init --hint "full pentest" --case-name "acme-2026"
```
...
## Evidence Graph review（handoff 前强制）

```bash
reverse-skill case review work/<case> --verify-hashes --strict
```

review 是只读的：检查 scope 字段、Evidence 记录、work item 与 timeline 引用、结构化
Finding、Path、以及 artifact hash 匹配。`--strict` 把 warning 视为 handoff 阻塞。
`artifact_path` 逃逸 case root 是 error（fail-closed），不是 warning。
