...
CORRECT (Agent 执行):
  "## 立即执行：先用 reverse-skill integrations 检查缺失工具，按需要手动安装缺失工具
...
| "我没找到 tool-index，我就直接猜路径" | **缺文件比猜错路径安全 100 倍。** tool-index 是旧快照；用 `reverse-skill route` / `integrations` 实时发现真实路径，禁止猜路径。 |
