...
from aigx_context import inspect_project as inspect_aigx_project, run_aigx_json
...
ROUTING_PATH = SKILLS_ROOT / "routing.json"
...
def is_ida_teams_task(task: Dict[str, Any]) -> bool:
    """Identify the explicit IDA 9.4 Teams Git-backend workflow."""
    text = task_text(task)
    return any(
...
def build_entrypoint(
    ...
    if module == "apk-reverse":
        script = SKILLS_ROOT / "apk-reverse" / "scripts" / "decode.ps1"
        if input_path:
            command = ["powershell.exe", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", str(script), "-ApkPath", input_path]
            reason = "controlled_apk_decode_entrypoint"
        ...
        if is_ida_teams_task(task):
            ...
            script_name = (
                "teams_worktree_lab.py"
                if is_teams_worktree_task(task)
                else ("teams_collaboration.py" if contract_path else "teams_preflight.py")
            )
            script = SKILLS_ROOT / "ida-reverse" / "scripts" / script_name
            if is_teams_worktree_task(task):
                python = shutil.which("python") or sys.executable
                reason = "controlled_teams_worktree_lab"
                if worktree_contract_path:
                    command = [python, str(script), "--contract", worktree_contract_path]
                    if task.get("teams_lab_apply"):
                        command.append("--apply")
            elif contract_path:
                python = shutil.which("python") or sys.executable
                command = [python, str(script), "--contract", contract_path]
                reason = "controlled_teams_collaboration_plan"
            elif repo_path:
                python = shutil.which("python") or sys.executable
                command = [python, str(script), "--repo", repo_path]
                reason = "controlled_git_ida_preflight"
        else:
            script = SKILLS_ROOT / "ida-reverse" / "scripts" / "open_idalib.py"
            if input_path:
                python = shutil.which("python") or sys.executable
                command = [python, str(script), "--path", input_path]
        ...
        if search_path and search_query:
            script = SKILLS_ROOT / "scripts" / "cli_search.py"
            python = shutil.which("python") or sys.executable
            command = [python, str(script), "--path", search_path, "--query", search_query]
        ...
        script = SKILLS_ROOT / "radare2" / "scripts" / "recon.ps1"
        if input_path:
            command = ["powershell.exe", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", str(script), "-TargetPath", input_path]
            reason = "controlled_radare2_recon_entrypoint"
