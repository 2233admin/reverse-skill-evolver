...
> 本文件只提供脱敏的通用方法参考，不记录当前目标，也不授予任何目标权限。
> 具体操作必须先由 `route_task.py` 验证当前 session 的输入、能力、授权范围与成功判据；未达到 `ready` 时停止执行。
...
> **使用边界**：以上命令是方法样例，不是当前目标授权。只有当前 route 为 `ready` 且操作位于显式 `authorization_scope` 内时才可执行；否则保持 blocked。
