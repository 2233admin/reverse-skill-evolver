...
def test_opencli_description_matches_click_command_surface() -> None:
    ...
    for command in description["command"]["commands"]:
        ...
        for nested in command.get("commands", []):
            assert f"  {nested['name']}" in command_help
            ...
            for option in nested.get("options", []):
                assert option["name"] in nested_help
            for argument in nested.get("arguments", []):
                assert argument["name"] in nested_help
